...
static int bpf_object_load_prog_native(struct bpf_object *obj, struct bpf_program *prog, __u8 *native,
...
retry_load:
	...
	if (log_level) {
		if (prog->log_buf) {
			...
			log_buf = obj->log_buf;
			log_buf_size = obj->log_size; own_log_buf = false; }
		else { log_buf_size = max((size_t)BPF_LOG_BUF_SIZE,
				log_buf_size * 2); tmp = realloc(log_buf,
					log_buf_size); if (!tmp) { ret =
					-ENOMEM; goto out; } log_buf = tmp;
				log_buf[0] = '\0'; own_log_buf = true; } }

	load_attr.log_buf = log_buf; load_attr.log_size = log_buf_size;
	load_attr.log_level = log_level; 
	load_attr.prog_precompiled = native;
	load_attr.prog_precompiled_sz = (__u8)500; //TODO: Change
...
static int
bpf_object__load_progs_native(struct bpf_object *obj, __u8 *native, int log_level)
{
	struct bpf_program *prog;
	size_t i;
	int err;
	...
	for (i = 0; i < obj->nr_programs; i++) {
		...
		err = bpf_object_load_prog_native(obj, prog, native, prog->insns, prog->insns_cnt,
					   obj->license, obj->kern_version, &prog->fd);
		if (err) {
			pr_warn("prog '%s': failed to load: %d\n", prog->name, err);
			return err;
...
static int bpf_object_load_native(struct bpf_object *obj, __u8 *native, int extra_log_level, const char *target_btf_path)
{
	int err, i;
	...
	err = err ? : bpf_object__create_maps(obj);
	err = err ? : bpf_object__relocate(obj, obj->btf_custom_path ? : target_btf_path);
	err = err ? : bpf_object__load_progs_native(obj, native, extra_log_level);
	err = err ? : bpf_object_init_prog_arrays(obj);
...
int bpf_object__load_native(struct bpf_object *obj, __u8 *native)
{
